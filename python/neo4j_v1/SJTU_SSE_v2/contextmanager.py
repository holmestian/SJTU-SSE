...
class Neo4jDBSessionManager:
    def __init__(self, uri, username, password, encrypted=True):
        self.uri = uri
        self.driver = self._get_db_driver(uri, username, password, encrypted)

    @staticmethod
    def _get_db_driver(uri, username, password, encrypted=True, max_pool_size=50, trust=TRUST_DEFAULT):
        """
        :param uri: Bolt uri
        :type uri: str
        ...
        """
        return GraphDatabase.driver(uri, auth=basic_auth(username, password), encrypted=encrypted, max_pool_size=max_pool_size, trust=trust)
